...
for i=1:numel(par_vacc)
    ...
    [T_tot_prev,~,~,I_prev_pwl2(:,i),~,D_prev_pwl2(:,i),~]=fSEIRDV_pwl_solver(PAR_prev_pwl,Lin_BETA(N_cal-31),T_tot_prev,Y0_prev_pwl,N,T_tot(N_cal-31),T_tot(N_cal));
...
for i=1:numel(par_vacc)
    ...
    [T_tot_prev,~,~,I_prev_pwe2(:,i),~,D_prev_pwe2(:,i),~]=fSEIRDV_pwe_solver(PAR_prev_pwe,Exp_BETA(N_cal-26),T_tot_prev,Y0_prev_pwl,N,T_tot(N_cal-26),T_tot(N_cal));
